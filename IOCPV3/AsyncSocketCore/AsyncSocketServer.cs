...
{
    public class AsyncSocketServer : IServer
    {
        ...
        private int m_socketTimeOutMS = 20*1000; //Socket最大超时时间，单位为MS
        ...
        public void Start()
        {
            IPEndPoint listenPoint = new IPEndPoint(IPAddress.Parse("0.0.0.0"), m_port);
            Start(listenPoint);
        }
        private void Start(IPEndPoint localEndPoint)
        {
            ...
            Program.Logger.Debug("Start listen socket {0} success", localEndPoint.ToString());
        ...
        void AcceptEventArg_Completed(object sender, SocketAsyncEventArgs acceptEventArgs)
        {
            ...
            {
                Program.Logger.Error("Accept client {0} error, message: {1}", acceptEventArgs.AcceptSocket, E.Message);
        ...
        {
            Program.Logger.Info("Client connection accepted. Local Address: {0}, Remote Address: {1}",
            ...
            {
                Program.Logger.Error("Accept client {0} error, message: {1}", userToken.ConnectSocket, E.Message);
                Program.Logger.Error(E.StackTrace);                
            }            
        ...
        void IO_Completed(object sender, SocketAsyncEventArgs asyncEventArgs)
        {
            ...
            {
                Program.Logger.Error("IO_Completed {0} error, message: {1}", userToken.ConnectSocket, E.Message);
                Program.Logger.Error(E.StackTrace);
            }                     
        }
        ...
        private void ProcessReceive(SocketAsyncEventArgs receiveEventArgs)
        {
            ...
            if (userToken.ReceiveEventArgs.BytesTransferred > 0 && userToken.ReceiveEventArgs.SocketError == SocketError.Success)
            {
                ...
                {
                    Program.Logger.Warn("Illegal client connection. Local Address: {0}, Remote Address: {1}", userToken.ConnectSocket.LocalEndPoint, 
        ...
        private void BuildingSocketInvokeElement(AsyncSocketUserToken userToken)
        {
            ...
            //} 
            userToken.AsyncSocketInvokeElement = new TCCTestProtocol(this,userToken);
        }
        ...
        public void CloseClientSocket(AsyncSocketUserToken userToken)
        {
            ...
            string socketInfo = string.Format("Local Address: {0} Remote Address: {1}", userToken.ConnectSocket.LocalEndPoint,
                userToken.ConnectSocket.RemoteEndPoint);
            Program.Logger.Info("Client connection disconnected. {0}", socketInfo);
            try
            {
            ...
            {
                Program.Logger.Error("CloseClientSocket Disconnect client {0} error, message: {1}", socketInfo, E.Message);
            }
